...
__global__ void inRange_kernel(const gpu::PtrStepSz<uchar3> src, gpu::PtrStepSzb dst, int lbc0, int ubc0, int lbc1, int ubc1, int lbc2, int ubc2) {
...
void inRange_gpu(gpu::GpuMat &src, Scalar &lowerb, Scalar &upperb, gpu::GpuMat &dst) {
  ...
  inRange_kernel<<<gridSize, blockSize>>>(src, dst, lowerb[0], upperb[0], lowerb[1], upperb[1], lowerb[2], upperb[2]);
}
